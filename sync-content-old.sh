...
sync_from_server() {
    ...
    # Content collections
    scp_to_server "thilacoloma:$SERVER_CONTENT_DIR/collections/" "$LOCAL_REPO_DIR/content/"
    
    # Globals
    scp_with_password -r "$SERVER:$REMOTE_PATH/content/globals/" "$LOCAL_PATH/content/"
    
    # Assets meta (if needed)
    if ssh_with_password "$SERVER" "[ -d '$REMOTE_PATH/content/assets' ]"; then
        scp_with_password -r "$SERVER:$REMOTE_PATH/content/assets/" "$LOCAL_PATH/content/" 2>/dev/null || true
    fi
...
sync_to_server() {
    ...
    ssh_with_password "$SERVER" "mkdir -p $REMOTE_PATH/backups/$(date '+%Y%m%d_%H%M%S') && cp -r $REMOTE_PATH/content $REMOTE_PATH/backups/$(date '+%Y%m%d_%H%M%S')/"
    
    # Upload content
    scp_with_password -r "$LOCAL_PATH/content/collections/" "$SERVER:$REMOTE_PATH/content/"
    scp_with_password -r "$LOCAL_PATH/content/globals/" "$SERVER:$REMOTE_PATH/content/"
    
    # Fix permissions
    ssh_with_password "$SERVER" "chown -R www-data:www-data $REMOTE_PATH/content && chmod -R 644 $REMOTE_PATH/content/**/*.md $REMOTE_PATH/content/**/*.yaml"
    
    # Clear caches
    ssh_with_password "$SERVER" "cd $REMOTE_PATH && php please cache:clear && php please view:clear"
...
check_differences() {
    ...
    scp_with_password -r "$SERVER:$REMOTE_PATH/content/" "$temp_dir/"
